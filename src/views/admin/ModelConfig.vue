<template>
    <div class="mc-model-config-container">
        <!-- 搜索栏 -->
        ...
        <div class="mc-table-container">
            <n-data-table :columns="columns" :data="configs" :loading="loading" :pagination="pagination"
                :bordered="false" size="small" remote />
        </div>
        ...
        <n-modal v-model:show="showAddModal" preset="dialog" title="配置信息" style="width: 800px;">
            ...
            <n-form ref="formRef" :model="formData" :rules="formRules" label-placement="left" label-width="120px"
                ...
                <div class="mc-form-grid">
                    ...
                    <n-form-item label="API域名" path="apiDomain" class="mc-full-width">
                    ...
                    <n-form-item label="API密钥" path="apiKey" class="mc-full-width">
                        <n-input v-model:value="formData.apiKey" type="password" placeholder="请输入API密钥"
                            show-password-on="click" />
                    </n-form-item>
                </div>

                <div class="mc-parameter-group">
                    <div class="mc-parameter-group-title">模型参数配置</div>
                    <div class="mc-form-grid">
                ...
                <div class="mc-form-grid">
                    ...
                    <n-form-item label="标签" path="caseTags" class="mc-full-width">
                        <n-input v-model:value="formData.caseTags" placeholder="请输入标签，多个标签用逗号分隔" />
                    </n-form-item>
                    <n-form-item label="简介" path="caseBrief" class="mc-full-width">
                        <n-input v-model:value="formData.caseBrief" type="textarea" placeholder="请输入简介"
                            :autosize="{ minRows: 2, maxRows: 4 }" />
                    </n-form-item>
                    <n-form-item label="备注" path="caseRemark" class="mc-full-width">
...
import '../../assets/css/model-config.css';
import {
    NButton,
    NInput,
    NInputNumber,
...
const getModelTypeTag = (type) => {
    const typeMap = {
        0: { label: '大模型', class: 'mc-model-type-0' },
        1: { label: '文本向量', class: 'mc-model-type-1' },
        2: { label: '视觉模型', class: 'mc-model-type-2' }
    };
    return typeMap[type] || { label: '未知', class: 'mc-model-type-0' };
};
...
const columns = [
    ...
    },
    {
        ...
        render(row) {
            ...
            return h(
                'span',
                {
                    class: `mc-model-type-tag ${typeInfo.class}`
                },
                typeInfo.label
            );
        }
    ...
    },
    {
        ...
        render(row) {
            return h(
                'div',
                { class: 'mc-api-key-display' },
                [
                    h(
                        'span',
                        { class: 'mc-api-key-text' },
